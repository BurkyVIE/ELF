...
p <- ggplot() +
  ...
  geom_label_repel(data, mapping = aes(x = Long, y = Lat, label = Franchise, fill = Division), colour = "black",
  ...
  scale_fill_manual(name = "Division", values = colors_elf$Division |> filter(Season == seas) |> select(Name, Color) |> deframe(), na.value = "#a39819") +
